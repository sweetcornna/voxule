...
- `scripts/run-sim.sh` —— iPhone 17 模拟器一键构建 + 安装 + 启动
...
App 构建 + 启模拟器（推荐走脚本，一句搞定构建 → 装 → 启）：

```bash
scripts/run-sim.sh             # 增量构建 + 装 + 启
scripts/run-sim.sh --fresh     # 卸载重装（清 NSUserDefaults / 本地 SwiftData）
scripts/run-sim.sh --build-only
DEVICE="iPhone 17 Pro" scripts/run-sim.sh
```

或直接 xcodebuild（免签名 —— 本机 SwiftPM 资源 bundle codesign 阶段会拒）：
